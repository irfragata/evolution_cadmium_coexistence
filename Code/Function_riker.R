...
# Function to modify the data frame structure to fit the optim design
...
######### Function for the optimization procedure, based on the approach used in Fragata et al 2022 and Matías et al 2018

# Riker function----

# This function estimates all model parameters. 

magic_rk<-function(filepath2,  data2, reps2, env2, comparisons, ...){
  
  # Function for optimization -----
  
  ### Fitting with negative binomial no competition
...
names(parameterList)<-c("alpha", "lower_alpha", "upper_alpha", "lambda", "lower_lambda", "upper_lambda", "sigma", "likelihood")
aux_vec_end<-NULL
...
for(i in 1:length(comparison_mat[,1])){
  ...
  for(k in 1:50){
    testcomp1<-optim(par1, compmodel0, data=aux_lograte,  hessian = TRUE, method = "BFGS")
    par1<-testcomp1$par
  ...
  for(k in 1:50){
    testcomp2<-optim(par2, compmodel1, dens=aux_dens, data=aux_lograte, l1=testcomp1$par[1], hessian = TRUE, method = "BFGS")
    par2<-testcomp2$par
  ...
  for(c in 2:length(background_list)){
    ...
    ## estimating interspecific
    for(k in 1:50){
      testcomp3<-optim(par3,data1=data_c, dens1=dens_intra,dens2=dens_inter, l1=testcomp1$par[1],i1=testcomp2$par[1], fn=compmodel2, hessian=T, method = "BFGS")
      par3<-testcomp3$par
      if(testcomp3$convergence==0){
        print(paste(comparison_mat[i,j], "model 3 converged on rep", k, sep=" "))
        break
    ...
    parameterList$convergence_code<-c(parameterList$convergence_code, testcomp3$value)
...
names(results_same_time)<-c("species", "lambda", "lower_error", "upper error", "sigma", "likelihood")
...
magic_rk_lambda<-function(filepath2,  data2, reps2, env2, comparisons, lam2, ...){
  ...
  for(i in 1:length(comparison_mat[,1])){
    ...
    ### estimating intraspecific interactions
    for(k in 1:50){
      testcomp2<-optim(par2, compmodel1, dens=aux_dens, data=aux_lograte, l1=subset(lam, SR==comparison_mat[i,1])$lambda, hessian = TRUE, method = "BFGS")
      par2<-testcomp2$par
      if(testcomp2$convergence==0){
        print(paste(comparison_mat[i,j],  "model 2 converged on rep", k, sep=" "))
        break
    ...
    for(c in 2:length(background_list)){
      ...
      ## estimating interspecific
      for(k in 1:50){
        testcomp3<-optim(par3,fn=compmodel2,data1=data_c,dens2=dens_inter, l1=subset(lam, SR==comparison_mat[i,1])$lambda, hessian=T, method = "BFGS")
        par3<-testcomp3$par
        if(testcomp3$convergence==0){
          print(paste(comparison_mat[i,j], "model 3 converged on rep", k, sep=" "))
          break

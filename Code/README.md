...
- Running_cxr_pooledData.R - code to estimate the parameters for the Riker model using pooled data. This code takes a while to run. Intermediate files with the data produced in this script are available in the Analyses folder. 
- Running_cxr_Replicates.R - code to estimate the parameters for the Riker model using data from each individual replicate. This code takes a long time to run. Intermediate files with the data produced in this script are available in the Analyses folder.

- Figures.R - This file has the whole code to obtain the figures produced for the paper. The code needs several intermediate files that are produced in the main analyses. These data files are available in the Analyses folder in case you want to run the Figures script without running the main analyes.
